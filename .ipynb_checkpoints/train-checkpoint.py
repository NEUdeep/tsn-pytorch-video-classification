...
def main():
    ...
    if args.DATA.input_type == 'Frames':
        from dataset.dataset_frames import TSNDataSetFrames
        ...
        from dataset.dataset_image import TSNDataSetImage
        train_loader = torch.utils.data.DataLoader(
        ...
        if args.DATA.video_handler == 'nvvl':
            from dataset.dataset_video_nvvl import TSNDataSetVideoNVVL
            ...
            from dataset.dataset_video_cv2 import TSNDataSetVideoCV2
            train_loader = torch.utils.data.DataLoader(
                TSNDataSetVideoCV2(args.DATA.train_list, num_segments=args.MODEL.num_segments,#args.DATA.train_list
                                   new_length=data_length,
                                   modality=args.DATA.modality,
                                   transform=train_transform),
                batch_size=args.MODEL.batch_size, shuffle=True,
    ...
    for epoch in range(args.MODEL.start_epoch, args.MODEL.epochs):
        adjust_learning_rate(optimizer, epoch, args.OPT.lr_step)
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if args.OPT.clip_gradient is not None:
            total_norm = clip_grad_norm(model.parameters(), args.OPT.clip_gradient)# args.OPT.clip_gradient is max_norm(float or int),nn.utils.clip_grad_norm(parameters, max_norm, norm_type=2)
